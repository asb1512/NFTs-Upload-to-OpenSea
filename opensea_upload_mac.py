    ...
    if attr_list[start_num - 1]["parler"] == "false":
        ...
        while end_num >= start_num:
            ...
            imagePath = os.path.abspath(
                file_path + "\\" + str(start_num) + "." + loop_file_format)  # change folder here
            ...
            wait_xpath(
                '//*[@id="__next"]/div[1]/main/div/div/div[1]/div/span[2]/a')
            sell = driver.find_element_by_xpath(
                '//*[@id="__next"]/div[1]/main/div/div/div[1]/div/span[2]/a')
            sell.click()

            wait_css_selector("input[placeholder='Amount']")
            amount = driver.find_element_by_css_selector(
...
button_start = tkinter.Button(
    root, width=20, bg="green", fg="white", text="Start", command=main_program_loop)
